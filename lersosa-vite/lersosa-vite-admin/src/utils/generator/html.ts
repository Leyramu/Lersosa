...
const layouts: any = {
    colFormItem(element: any): string {
        let labelWidth = '';
    ...
    rowFormItem(element: any): string {
...
const tags: any = {
    ...
    'el-checkbox-group': (el: any): string => {
        const {disabled, vModel} = attrBuilder(el);
        const size = `size="${el.size}"`;
        const min = el.min ? `:min="${el.min}"` : '';
        const max = el.max ? `:max="${el.max}"` : '';
...
function attrBuilder(el: any): any {
    return {
...
// el-buttin 子级
function buildElButtonChild(conf: any): string {
...
// el-input innerHTML
function buildElInputChild(conf: any): string {
    const children: string[] = [];
    if (conf.prepend) {
...
function buildElSelectChild(conf: any): string {
    const children: string[] = [];
...
function buildElRadioGroupChild(conf: any): string {
    const children: string[] = [];
    if (conf.options && conf.options.length) {
...
function buildElCheckboxGroupChild(conf: any): string {
    const children: string[] = [];
    if (conf.options && conf.options.length) {
...
function buildElUploadChild(conf: any): string {
    const list: string[] = [];
...
export function makeUpHtml(conf: any, type: 'dialog' | 'form'): string {
    const htmlList: string[] = [];
    confGlobal = conf;
    // @ts-ignore
    const someSpanIsNot24 = conf.fields.some((item: any) => item.span !== 24);
