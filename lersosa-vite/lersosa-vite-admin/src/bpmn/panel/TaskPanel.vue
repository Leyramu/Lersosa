...
const openDueDate = (_e: any) => {
  dueDateRef.value.openDialog();
};
const blurAssignee = (assignee: any) => {
...
const singleUserSelectCallBack = (data: UserVO[]) => {
  ...
  extensionElements.values = extensionElements.get('values').filter((item: any) => item.$type !== 'flowable:extAssignee');
  if (user) {
...
const userSelectCallBack = (data: UserVO[]) => {
  let extensionElements = getExtensionElements();
  extensionElements.values = extensionElements.values.filter((item: any) => item.$type !== 'flowable:extCandidateUsers');
  if (data.length === 0) {
...
const collectionChange = (newVal: any) => {
  let loopCharacteristics = props.element.businessObject.get('loopCharacteristics');
  if (!loopCharacteristics) {
    loopCharacteristics = createModdleElement('bpmn:MultiInstanceLoopCharacteristics', {}, props.element.businessObject);
  }
...
const elementVariableChange = (newVal: any) => {
  let loopCharacteristics = props.element.businessObject.get('loopCharacteristics');
  if (!loopCharacteristics) {
    loopCharacteristics = createModdleElement('bpmn:MultiInstanceLoopCharacteristics', {}, props.element.businessObject);
  }
...
const completionConditionChange = (newVal: any) => {
...
const dueDateChange = (newVal: any) => {
...
onBeforeMount(() => {
  ...
  if (extensionElements && extensionElements.get('values')) {
    let extAssigneeElement = extensionElements.get('values').find((item: any) => item.$type === 'flowable:extAssignee');
