...
# 服务器配置类
class ServerConfig:
    """服务器配置类，用于管理服务端网络配置参数
    Attributes:
        SERVER_IP (str): 服务端监听IP地址，优先从环境变量SERVER_IP获取，
            未设置时默认使用 'localhost'
        SERVER_PORT (int): 服务端监听端口号，优先从环境变量SERVER_PORT获取，
            未设置时默认使用8000，环境变量值会被转换为整数类型
    """
    SERVER_IP: str = os.getenv('SERVER_IP', 'lersosa-python')
