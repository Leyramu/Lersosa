...
class ComponentAnnotation:
    """@Component注解 - 标记一个类为Spring管理的组件

    这是最基础的注解，@Service、@Controller都基于此注解
    """
    ...
    def create(name: str = "", singleton: bool = True, lazy_init: bool = False) -> Callable[[type], type]:
        ...
        Notes
            @ComponentAnnotation.create()
            class MyClass:
                pass

            @ComponentAnnotation.create(name="myCustomBean")
            class MyClass:
                pass
        """
        ...
        def decorator(cls: type) -> type:
            ...
            logger.info("@Component: 注册组件 '%s' -> '%s'", cls.__name__, bean_name)
            return cls
