#  Copyright (c) 2020-2026 Leyramu Group. All rights reserved.
#  Licensed under the Apache License, Version 2.0 (the "License");
#  you may not use this file except in compliance with the License.
#  You may obtain a copy of the License at
#
#       http://www.apache.org/licenses/LICENSE-2.0
#
#  Unless required by applicable law or agreed to in writing, software
#  distributed under the License is distributed on an "AS IS" BASIS,
#  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
#  See the License for the specific language governing permissions and
#  limitations under the License.
#
#  This project (Lersosa), including its source code, documentation,
#  and any associated materials, is the intellectual property of Leyramu.
#  No part of this software may be reproduced, distributed,
#  or transmitted in any form or by any means, including photocopying,
#  recording, or other electronic or mechanical methods,
#  without the prior written permission of the copyright owner,
#  Miraitowa_zcx, except in the case of brief quotations embodied in
#   critical reviews and certain other noncommercial uses permitted by copyright law.
#
#  For inquiries related to licensing or usage outside the scope of this notice,
#  please contact the copyright holder at 2038322151@qq.com.
#
#  The author disclaims all warranties, express or implied,
#  including but not limited to the warranties of merchantability and
#  fitness for a particular purpose. Under no circumstances shall the author
#  be liable for any special, incidental, indirect, or consequential damages
#  arising from the use of this software.
#
#  By using this project, users acknowledge and agree to abide by these terms and conditions.


from .annotation_registry import AnnotationRegistry
from .bean_annotation import BeanAnnotation
from .component_annotation import ComponentAnnotation
from .component_metadata import ComponentMetadata
from .controller_annotation import ControllerAnnotation
from .service_annotation import ServiceAnnotation
from .slf4j_annotation import Slf4jAnnotation


# 智能包装器
class _SmartDecorator:
    """智能装饰器包装器
        支持带参数和不带参数的用法
    """

    def __init__(self, factory_func):
        self._factory = factory_func

    def __call__(self, *args, **kwargs):
        # 如果第一个参数是类，说明是 @Decorator 形式（无括号）
        if args and len(args) == 1 and isinstance(args[0], type) and not kwargs:
            return self._factory()(args[0])
        # 否则是 @Decorator() 或 @Decorator(args) 形式
        return self._factory(*args, **kwargs)


# 使用智能包装器
Component = _SmartDecorator(ComponentAnnotation.create)
Service = _SmartDecorator(ServiceAnnotation.create)
RestController = _SmartDecorator(ControllerAnnotation.create)
Bean = _SmartDecorator(BeanAnnotation.create)
Slf4j = _SmartDecorator(Slf4jAnnotation.create)

__all__ = [
    "AnnotationRegistry",
    "ComponentMetadata",
    "ComponentAnnotation",
    "ServiceAnnotation",
    "ControllerAnnotation",
    "BeanAnnotation",
    "Slf4jAnnotation",
    "Component",
    "Service",
    "RestController",
    "Bean",
    "Slf4j",
]
