...
class BeanAnnotation:
    """@Bean注解 - 标记一个类为由容器管理的Bean

    这是最通用的注解，允许完全自定义Bean的配置
    """

    @staticmethod
    def create(name: str = "", singleton: bool = True, lazy_init: bool = False,
               dependencies: list[str] | None = None) -> Callable[[type], type]:
        """创建@Bean装饰器
        ...
        Notes:
            @BeanAnnotation.create()
            class MyBean:
                pass

            @BeanAnnotation.create(name="customBean", dependencies=["otherBean"])
            class MyBean:
                pass
        """
        ...
        def decorator(cls: type) -> type:
            ...
            logger.info("@Bean: 注册Bean '%s' -> '%s'", cls.__name__, bean_name)
            return cls
