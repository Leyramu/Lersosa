...
from app.core.state.lifecycle_manager import LifecycleManager


class NacosLifecycle(LifecycleManager):
    """Nacos生命周期管理器，管理FastAPI应用与Nacos服务发现的集成"""
    ...
    def __init__(self, app: FastAPI) -> None:
        ...
        self._nacos_client = NacosClientWrapper(
        ...
        self._service_name = NacosConfig.NACOS_SERVICE_NAME
        self._group_name = NacosConfig.NACOS_GROUP
        self._ip = ServerConfig.SERVER_IP
        self._port = ServerConfig.SERVER_PORT
        self._heartbeat_interval = NacosConfig.HEARTBEAT_INTERVAL
    ...
    async def _on_startup(self) -> None:
        ...
        # 注册服务到Nacos
        await self._nacos_client.register_service(
            self._service_name, self._group_name, self._ip, self._port
        )

        # 启动心跳任务
        self._heartbeat_task = asyncio.create_task(
            self._nacos_client.service_heartbeat(
                self._service_name,
                self._group_name,
                self._ip,
                self._port,
    ...
    async def _on_shutdown(self) -> None:
        ...
        if self._heartbeat_task:
            ...
            try:
                # 等待心跳任务完成或被取消
                await asyncio.wait_for(self._heartbeat_task, timeout=1)
            except (asyncio.CancelledError, asyncio.TimeoutError):
                # 如果心跳任务被取消或等待超时，忽略错误
                pass

        # 注销服务
        await self._nacos_client.unregister_service(
            self._service_name, self._group_name, self._ip, self._port
        )
