...
class RedisService:
    ...
    async def get_task_result(self, task_id: str) -> Optional[str]:
        return await self.redis_client.get(f"task:{task_id}")

    async def save_task_result(self, task_id: str, result: str, ttl: int = 300):
        await self.redis_client.set(f"task:{task_id}", result, ex=ttl)

    async def is_task_exists(self, task_id: str) -> bool:
        return await self.redis_client.exists(f"task:{task_id}")
