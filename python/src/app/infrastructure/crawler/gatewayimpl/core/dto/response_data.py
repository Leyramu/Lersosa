...
class CrawlerResponseData(BaseModel):
    """爬虫响应数据
    ...
    url: str = Field(..., description="响应 URL")
    status_code: int = Field(..., description="HTTP 状态码")
    elapsed_seconds: float = Field(..., description="请求耗时（秒）")
    text: Optional[str] = Field(default=None, description="响应文本内容")
    payload: Optional[dict[str, Any]] = Field(default=None, description="响应载荷数据")
    parsed_records: List[dict[str, Any]] = Field(default_factory=list, description="解析后的记录列表")
