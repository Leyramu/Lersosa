...
class RobotsProtocolStrategyI(ABC):
    """robots.txt 协议处理策略接口

    Notes：
        - 策略模式：不同场景使用不同的 robots.txt 处理实现
        - 开闭原则：可以扩展新的协议处理策略
        - 依赖倒置：上层依赖此接口而非具体实现
    """
    ...
    @staticmethod
    def get_strategy(strategy_name: str, config: Any = None) -> "RobotsProtocolStrategyI":
        """根据策略名称获取对应的策略实例
        ...
        all_strategies = bean_container.get_beans_by_type(RobotsProtocolStrategyI)
