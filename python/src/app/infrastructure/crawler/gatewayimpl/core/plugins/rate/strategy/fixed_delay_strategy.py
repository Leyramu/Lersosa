...
@Component
class FixedDelayStrategy(RateControlStrategyI):
    """固定延迟速率控制策略"""

    def __init__(self, config: Any = None):
        if config:
            self._base_delay = getattr(config, 'base_delay', 1.0)
            jitter_min = getattr(config, 'jitter_min', 0.0)
            jitter_max = getattr(config, 'jitter_max', 0.5)
            self._jitter_range = (jitter_min, jitter_max)
        else:
            self._base_delay = 1.0
            self._jitter_range = (0.0, 0.5)
    ...
    def wait_before_request(self, url: str) -> None:
        ...
        if elapsed < self._base_delay:
            wait_time = self._base_delay - elapsed
            if self._jitter_range[1] > 0:
                jitter = random.uniform(self._jitter_range[0], self._jitter_range[1])
                wait_time += jitter
