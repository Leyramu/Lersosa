...
class CrawlerPipelineHandler(ABC):
    """爬虫责任链处理器接口"""

    def __init__(self):
        self._next_handler: CrawlerPipelineHandler | None = None

    def set_next(self, handler: CrawlerPipelineHandler) -> "CrawlerPipelineHandler":
        """设置下一个处理器
    ...
    def handle(self, ctx: CrawlerRequestContext, **kwargs: Any) -> CrawlerResponseData | None:
        ...
        Returns:
            响应数据对象，如果返回 None 则表示被拦截或交由后续处理
        """
        pass

    def _handle_next(self, ctx: CrawlerRequestContext, **kwargs: Any) -> CrawlerResponseData | None:
        """调用下一个处理器"""
        if self._next_handler:
            return self._next_handler.handle(ctx, **kwargs)
        return None
