...
import leyramu.framework.lersosa.auth.gateway.TenantGateway;
...
public class TenantGatewayImpl implements TenantGateway {
     ...
     */
    @Override
    public TenantE queryByTenantId(String tenantId) {
        return TenantConvertor.toDomainObject(remoteTenantService.queryByTenantId(tenantId));
    }
    ...
    public List<TenantE> queryList() {
        return remoteTenantService.queryList().parallelStream().map(TenantConvertor::toDomainObject).toList();
    }
}
