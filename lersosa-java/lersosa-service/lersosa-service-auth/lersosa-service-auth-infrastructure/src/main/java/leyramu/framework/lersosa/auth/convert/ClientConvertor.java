...
public class ClientConvertor {
     ...
     * 将远程客户端对象转换为客户端值对象.
     *
     * @param clientVo 远程客户端对象
     * @return 客户端对象
     */
    public static ClientV toDomainObject(RemoteClientVo clientVo) {
        return new ClientV(
            clientVo.getClientId(),
            clientVo.getClientKey(),
            clientVo.getGrantType(),
            clientVo.getDeviceType(),
            clientVo.getActiveTimeout(),
            clientVo.getTimeout(),
            clientVo.getStatus()
        );
    }
