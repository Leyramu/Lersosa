...
import leyramu.framework.lersosa.auth.model.form.PasswordLoginBody;
 ...
 */
@Slf4j
@Service("password" + AuthStrategyI.BASE_NAME)
@RequiredArgsConstructor
public class PasswordAuthStrategyImpl implements AuthStrategyI {
    ...
    public LoginCo login(String body, ClientV clientV) {
        ...
        LoginUser loginUser = TenantHelper.dynamic(tenantId, () -> {
            ...
            authDomainService.checkLogin(LoginType.PASSWORD, tenantId, username, () -> !BCrypt.checkpw(password, user.getPassword()));
            return user;
        });
    ...
    private void validateCaptcha(String tenantId, String username, String code, String uuid) {
        ...
        if (captcha == null) {
            authDomainService.recordLogininfor(tenantId, username, Constants.LOGIN_FAIL, MessageUtils.message("user.jcaptcha.expire"));
            throw new CaptchaExpireException();
        }
        if (!code.equalsIgnoreCase(captcha)) {
            authDomainService.recordLogininfor(tenantId, username, Constants.LOGIN_FAIL, MessageUtils.message("user.jcaptcha.error"));
